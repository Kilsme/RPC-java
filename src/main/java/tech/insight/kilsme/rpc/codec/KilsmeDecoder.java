...
public class KilsmeDecoder extends LengthFieldBasedFrameDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        try {
            ...
            if (compression == null) {
                throw new IllegalArgumentException("不支持的压缩类型" + (serializerAndCompress & 0b00001111));
            }
            Serializer serializer = this.serializerManager.getSerializer((serializerAndCompress & 0b11110000) >>> 4);
            if (serializer == null) {
                throw new IllegalAccessException("不支持的序列化类型" + ((serializerAndCompress & 0b11110000) >>> 4));
            }
    ...
    private void initIfNecessary(ChannelHandlerContext context) {
        if (serializerManager != null) return;

        serializerManager = context.channel().attr(KilsmeEncoder.SERIALIZE_MANAGER_KEY).get();
        compressionManager = context.channel().attr(KilsmeEncoder.COMPRESS_MANAGER_KEY).get();
    }
